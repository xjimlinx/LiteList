...
路线会按依赖层级自动排列并绘制分支连线。标题和说明会根据节点尺寸显示多行，仍放不下时悬停节点可查看完整文字。全局设置可以选择默认节点形状，新建或编辑小目标时也能为单个节点指定不同形状。节点层数增多时，路线会按实际高度展开并由目标页面统一滚动；横向分支超过可见宽度时，可拖动科技树空白处或使用底部滚动条浏览。一个节点被其他节点依赖时不能直接删除；已有完成节点依赖某项时，也不能贸然撤回该前置项。普通待办仍保持扁平、快速的操作方式，不会被强制放入目标树。
